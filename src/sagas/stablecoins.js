...
import { ETHWeb3, STABLECOIN_ADDRESS } from '../bootstrap/dapp-api'
...
export function* fetchStablecoins() {
  ...
  const StablecoinBadgeInstance = new ETHWeb3.eth.Contract(
  ...
  while (moreStablecoins) {
    const query = yield call(
      StablecoinBadgeInstance.methods.queryAddresses(
        lastAddress,
        1000,
    ...
    lastAddress =
      stablecoins.length > 0 ? stablecoins[stablecoins.length - 1] : lastAddress
  }
